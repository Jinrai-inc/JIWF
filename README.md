...
### パターンを更新した後に既存ページを書き直す

ブロックパターン（about-starter など）を更新したけど、既存の固定ページの本文に反映されないとき：

**`/wp-admin/?jiwf_seed=force`** に管理者でアクセスすると、シードしたページ（About / Locations / Community / Contact / Privacy / Terms）の本文を最新のパターン内容で**上書き**します。タイトル・スラッグ・ID は保持されるので、メニューや既存リンクは壊れません。

> ⚠ 編集者が手で書き直したコピーがある場合、上書きされて消えます。バックアップを取ってから実行してください。
